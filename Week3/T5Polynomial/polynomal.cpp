...
private:
	...
	private:
		...
		Polynomial& pol;
	public:
		ProxyIndex(size_t pos, Polynomial& p) : pos(pos), pol(p) {}

		ProxyIndex& operator=(T elem)
		{
			...
			if (pos >= pol.coeffs_.size())
			{
				pol.coeffs_.resize(pos + 1);
			}
			pol.coeffs_[pos] = elem;
			return *this;
		}

		operator T() const
		{
			return as_const(pol)[pos];
		}
	};
